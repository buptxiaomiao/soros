...
create table if not exists l1.fact_stock_money_flow (
    ...
    total_amount        float       comment '总成交额（万元）',
    avg_vol_per_trade   float       comment '单笔交易手数',
    net_in_gte_lg_amount   float    comment '大单及以上净流入金额（万元）',
    net_in_gte_lg_amount_last2  float   comment '近2日大单净流入金额（万元）',
    net_in_gte_lg_amount_last5  float   comment '近5日大单净流入金额（万元）',
    net_in_gte_lg_amount_last10 float   comment '近10日大单净流入金额（万元）',
    ...
    but_gte_lg_amount   float       comment '大单及以上买入金额（万元）',
    sell_gte_lg_vol     int         comment '大单及以上卖出量（手）',
    sell_gte_lg_amount  float       comment '大单及以上卖出金额（万元）',
    net_in_gte_lg_vol   int         comment '大单及以上净流入量（手）',

    net_in_vol          int         comment '净流入量（手）',
    net_in_amount       float       comment '净流入额（万元）',
